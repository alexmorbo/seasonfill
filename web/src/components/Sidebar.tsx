...
function NavList({ onNavigate }: { onNavigate?: () => void }) {
  ...
  return (
    <nav className="flex flex-col gap-0.5 h-full">
      <div className="px-2.5 pt-2 pb-1.5 text-[11px] uppercase tracking-[0.08em] text-faint">Navigation</div>
      {NAV.map((item) => (
      ...
      <Button type="button" onClick={() => { navigate('/scans?new=1'); onNavigate?.(); }}
