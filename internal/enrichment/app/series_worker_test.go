...
// MarkRecsSynced — E-1 A3b narrow refresh stamp.
func (f *fakeSeriesRepo) MarkRecsSynced(ctx context.Context, id domain.SeriesID, now time.Time) error {
	f.rec.add("Series.MarkRecsSynced")
	if c, ok := f.rows[id]; ok {
		t := now
		c.EnrichmentRecsSyncedAt = &t
		f.rows[id] = c
	}
	return nil
}
...
func (f *fakeSeriesRepo) UpsertStub(ctx context.Context, c series.Canon) (domain.SeriesID, error) {
	...
	if c.TMDBID != nil {
		if existingID, ok := f.byTMDB[int(*c.TMDBID)]; ok {
			...
			// S-E3a — canon no longer carries poster/backdrop; those columns
			// (and their COALESCE preservation) moved to series_media_texts.
			c.ID = existing.ID
...
func (f *workerFixture) seedCanon(id domain.SeriesID, tmdbID *domain.TMDBID) {
	f.series.rows[id] = series.Canon{ID: id, TMDBID: tmdbID, OriginalTitle: new("Existing"), InProduction: true}
}
...
// TestSeriesWorker_RecommendationStub_PreservesFullHydration: Story 319
// regression — a recommendation sweep MUST NOT downgrade an existing 'full'
// canon row to 'stub' when the stub payload references it. S-E3a moved
// poster_asset/backdrop_asset off canon into series_media_texts, so the only
// canon-column invariant this sweep now protects is the hydration level.
func TestSeriesWorker_RecommendationStub_PreservesFullHydration(t *testing.T) {
	t.Parallel()
	tv := minimalTV()
	f := newWorkerFixture(t, tv, map[int]*tmdb.SeasonResponse{1: minimalSeason()})
	tmdbID := domain.TMDBID(42)
	...
	// After the recommendation sweep, the row's full hydration MUST survive.
	got := f.series.rows[2]
...
// S-E3a — the Story 346 applyAll canon-image tests
// (LogsCanonImagesPersisted / DefensiveBackdropWriteGuard / GuardSkipsWhenTMDBEmpty /
// PreservesExistingBackdrop) were removed: applyAll no longer writes canon
// poster_asset/backdrop_asset nor emits the images_persisted diagnostic. Series art
// now flows to series_media_texts (RefreshText) and the media pipeline
// (RefreshMediaAssets) — see series_worker_refresh_media_assets_test.go.
