...
// TestPersonCreditsRepository_ListByPersonWithTextFallback_D0 — #1032 person
// page localization. Same requested-lang → en-US → base character_name
// fallback as the media reader, but scoped by person_id (the person filmography
// list). Two LEFT JOINs on person_credits_texts + COALESCE, one round-trip.
func TestPersonCreditsRepository_ListByPersonWithTextFallback_D0(t *testing.T) {
	t.Parallel()
	for _, backend := range testhelpers.AllBackends(t) {
		t.Run(backend.Name, func(t *testing.T) {
			t.Parallel()
			...
			// seed installs one tv credit for a fresh person whose base
			// character_name is baseName plus any texts rows in seedTexts, and
			// returns the repo + the owning person_id.
			seed := func(t *testing.T, baseName string, seedTexts []people.PersonCreditText) (*PersonCreditsRepository, int64) {
				t.Helper()
				db := backend.NewDB(t)
				personID, err := NewPeopleRepository(db).Upsert(ctx, samplePerson("Reader Actor"))
				require.NoError(t, err)
				credit := samplePersonCredit(personID, "credit-person-i18n-1", "R&M", 100)
				credit.CharacterName = new(baseName)
				pcRepo := NewPersonCreditsRepository(db)
				creditID, err := pcRepo.Upsert(ctx, credit)
				require.NoError(t, err)
				...
				return pcRepo, personID
			}

			// Case A — ru exists → returns ru.
			t.Run("ru_present_returns_ru", func(t *testing.T) {
				repo, personID := seed(t, "Morty", []people.PersonCreditText{
					{Language: "en-US", CharacterName: new("Rick")},
					{Language: "ru-RU", CharacterName: new("Рик")},
				})
				rows, err := repo.ListByPersonWithTextFallback(ctx, personID, "ru-RU")
				require.NoError(t, err)
				require.Len(t, rows, 1)
				require.NotNil(t, rows[0].CharacterName)
				assert.Equal(t, "Рик", *rows[0].CharacterName)
			...
			// Case C — no texts rows → base person_credits.character_name.
			t.Run("no_texts_returns_base", func(t *testing.T) {
				repo, personID := seed(t, "Morty", nil)
				rows, err := repo.ListByPersonWithTextFallback(ctx, personID, "ru-RU")
				require.NoError(t, err)
				require.Len(t, rows, 1)
				require.NotNil(t, rows[0].CharacterName)
			...
			t.Run("empty_lang_defaults_to_en", func(t *testing.T) {
				...
				rows, err := repo.ListByPersonWithTextFallback(ctx, personID, "")
				require.NoError(t, err)
				require.Len(t, rows, 1)
				require.NotNil(t, rows[0].CharacterName)
				assert.Equal(t, "Rick", *rows[0].CharacterName)
			...
			// Empty result — unknown person_id → zero rows, no error.
			t.Run("unknown_person_returns_empty", func(t *testing.T) {
				repo, _ := seed(t, "Morty", nil)
				rows, err := repo.ListByPersonWithTextFallback(ctx, 999999, "ru-RU")
				require.NoError(t, err)
				assert.Empty(t, rows)
			})
		})
