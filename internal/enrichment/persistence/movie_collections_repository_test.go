...
func TestMovieCollectionsRepository_GetLocalized(t *testing.T) {
	...
	for _, backend := range testhelpers.AllBackends(t) {
		t.Run(backend.Name, func(t *testing.T) {
			...
			now := time.Now().UTC()
			require.NoError(t, texts.UpsertCollectionTexts(ctx, cid, "ru-RU", "Дюна: Коллекция", "Эпическая сага.", nil, now))
			require.NoError(t, texts.UpsertCollectionTexts(ctx, cid, "en-US", "Dune Collection (EN)", "English overview.", nil, now))
