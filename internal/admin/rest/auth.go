...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if h.limiter != nil && !h.limiter.Allow(ip) {
		h.logger.WarnContext(c.Request.Context(), "auth.login.rate_limited",
			slog.String("ip", ip))
		observability.AuthLogin(observability.AuthLoginModeForms, observability.AuthLoginRateLimited)
	...
	if !auth.ConstantLatencyVerify(hashToCompare, password) {
		h.logger.WarnContext(c.Request.Context(), "auth.login.failed",
			slog.String("ip", ip))
		observability.AuthLogin(observability.AuthLoginModeForms, observability.AuthLoginFailure)
	...
	h.logger.InfoContext(c.Request.Context(), "auth.login.success",
		slog.String("username", user.Username))
	observability.AuthLogin(observability.AuthLoginModeForms, observability.AuthLoginSuccess)
