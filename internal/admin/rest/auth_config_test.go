...
func TestAuthConfig_ReturnsModeForms(t *testing.T) {
	t.Parallel()
	r := setupAuthConfig(t, &middleware.AuthRuntime{Mode: runtime.AuthModeForms})
...
// {mode, oidc_ready}. Adding fields here is intentional churn — keep the
...
func TestAuthConfig_NoSecretsLeaked(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(w.Body.Bytes(), &body))
	assert.Equal(t, 2, len(body), "auth_config wire shape must be exactly {mode, oidc_ready}")
