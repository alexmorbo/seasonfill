...
func (r *FollowedSeriesRepository) Follow(ctx context.Context, seriesID domain.SeriesID) error {
	...
	if res.Error != nil {
		return fmt.Errorf("follow series %d: %w", int64(seriesID), res.Error)
	}
	return nil
}
...
func (r *FollowedSeriesRepository) Unfollow(ctx context.Context, seriesID domain.SeriesID) error {
	err := dbtx.DBFromContext(ctx, r.db).WithContext(ctx).
		Where("series_id = ?", int64(seriesID)).
		Delete(&database.FollowedSeriesModel{}).Error
	if err != nil {
		return fmt.Errorf("unfollow series %d: %w", int64(seriesID), err)
	}
	return nil
}
...
func (r *FollowedSeriesRepository) ListFollowed(ctx context.Context, lang string) ([]followapp.FollowedItem, error) {
	const q = `
SELECT s.id            AS series_id,
 ...
 LIMIT 500`
	...
	if err := dbtx.DBFromContext(ctx, r.db).WithContext(ctx).
		Raw(q, lang, lang).Scan(&rows).Error; err != nil {
