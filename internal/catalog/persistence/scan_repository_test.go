...
// Story A-4-3b-7 (2026-06-20): legacy TestMain / setupTestDB / schema-cache
// machinery removed. The pre-A-4 schema-cache shortcut (story 314) is
// superseded by testhelpers.AllBackends — SQLite path runs the full
// migration via `newSQLiteDB`, Postgres path memoizes the container via
// `StartPostgres`. Both are fast enough for the current test count.
