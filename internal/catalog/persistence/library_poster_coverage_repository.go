...
// Covered = non-deleted library series (series_cache) that carry a
// series_media_texts row with a non-NULL poster_asset; Total = all
// non-deleted library series.
...
// dialect-portable (plain COUNT + EXISTS, no FILTER) so it runs identically
// on Postgres (prod) and the SQLite test lane.
func (r *LibraryPosterCoverageRepository) LibraryPosterCoverage(ctx context.Context) (LibraryPosterCoverage, error) {
	...
	const query = `SELECT
	  (SELECT COUNT(*) FROM series_cache WHERE deleted_at IS NULL) AS total,
	  (SELECT COUNT(*) FROM series_cache sc
