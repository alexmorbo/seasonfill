...
// TestCounterRepository_ExplainUsesIndex_SQLite is SQLite-only by
// design — `EXPLAIN QUERY PLAN` is SQLite-specific syntax. The test
// guards against an unintended drop of the composite index used by the
// hot counter aggregation. Postgres has its own EXPLAIN dialect; if a
...
func TestCounterRepository_ExplainUsesIndex_SQLite(t *testing.T) {
	...
	found := false
	for _, p := range plan {
		if strings.Contains(p.Detail, "grab_records_inst_created_idx") {
			found = true
			break
		}
	}
	assert.True(t, found, "expected grab_records_inst_created_idx in plan: %+v", plan)
}
