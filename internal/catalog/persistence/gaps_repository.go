...
// GapEpisodes — bounded, ordered drill-down of gap episodes for the
// instance. Each row carries the per-(series, season) aired-monitored +
// missing totals via correlated scalar subqueries (dialect-portable, no
// N+1: one query returns everything). Bind order:
...
//   - limit
func (r *GapRepository) GapEpisodes(ctx context.Context, instance string, now time.Time, limit int) ([]ports.GapEpisodeRow, error) {
	const sqlText = `
	...
	if err := dbFromContext(ctx, r.db).WithContext(ctx).
		...
		return nil, fmt.Errorf("gaps gap_episodes: %w", err)
	}
