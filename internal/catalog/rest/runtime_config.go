...
func dtoToInput(d dto.RuntimeConfigDTO) (runtimeconfig.Input, error) {
	...
	return runtimeconfig.Input{
		...
		GUIDRewrites: dtoToRewrites(d.GUIDRewrites),
	}, nil
}

// dtoToRewrites converts the wire rule slice to the runtime form. nil
// body → nil slice (the usecase validate step makes it non-nil after).
func dtoToRewrites(in []dto.GUIDRewriteDTO) []runtime.GUIDRewriteRule {
	if in == nil {
		return nil
	}
	out := make([]runtime.GUIDRewriteRule, 0, len(in))
	for _, r := range in {
		out = append(out, runtime.GUIDRewriteRule{From: r.From, To: r.To})
	}
	return out
}

// rewritesToDTO converts the typed form to the wire form. Always
// non-nil (the wire contract is `[]` not `null` even for empty).
func rewritesToDTO(in []runtime.GUIDRewriteRule) []dto.GUIDRewriteDTO {
	out := make([]dto.GUIDRewriteDTO, 0, len(in))
	for _, r := range in {
		out = append(out, dto.GUIDRewriteDTO{From: r.From, To: r.To})
	}
	return out
}
...
func outputToDTO(out runtimeconfig.Output) dto.RuntimeConfigDTO {
	return dto.RuntimeConfigDTO{
		...
		GUIDRewrites:        rewritesToDTO(out.GUIDRewrites),
