...
// Copied from interface/http/handlers/fakes_sonarr_test.go during story
// 431 (A-1-5) when the grab handlers moved into internal/grab/rest. The
// admin/rest test tree carries its own copy under that path — duplication
// is deliberate: each test package is an independent compilation unit
// and the fake has no behaviour worth centralising into a shared kernel
// helper package.
