...
func TestRC_Get_ErrorPath(t *testing.T) {
	t.Parallel()
	repo := &rcFakeRuntime{exists: true, row: ports.RuntimeConfigRow{
		Cron: runtime.CronSnapshot{Schedule: "0 */6 * * *"},
	}}
	// Make Get return an internal error by swapping the repo out of bounds.
	// Easiest: wrap with an erroring usecase by using a custom repo type.
	// We reuse rcBrokenRuntime defined below.
	...
	_ = repo // referenced to prevent unused-import lint warning
}
