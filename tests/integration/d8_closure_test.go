...
// expected version (32) with dirty=false. golang-migrate stores a
// single "current head" row (not a per-migration history), so the
// invariant is MAX(version)=32 AND dirty=false. Catches the "I forgot
		...
		t.Run(b.name, func(t *testing.T) {
			...
			assert.Equalf(t, wantHead, head,
				"schema_migrations.version on %s = %d; want %d "+
					"(matches the 32 .up.sql files committed in 000001..000032)",
