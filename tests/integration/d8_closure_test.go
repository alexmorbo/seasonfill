...
// expected version (37) with dirty=false. golang-migrate stores a
// single "current head" row (not a per-migration history), so the
// invariant is MAX(version)=37 AND dirty=false. Catches the "I forgot
		...
		t.Run(b.name, func(t *testing.T) {
			...
			assert.Equalf(t, wantHead, head,
				"schema_migrations.version on %s = %d; want %d "+
					"(matches the 37 .up.sql files committed in 000001..000037)",
