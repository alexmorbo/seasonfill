...
//
//  1. All 24 migrations apply cleanly from an empty DB (no compound
...
//  4. schema_migrations reports exactly 24 applied versions —
//     matches the 24 .up.sql files under
...
// expected version (24) with dirty=false. golang-migrate stores a
// single "current head" row (not a per-migration history), so the
// invariant is MAX(version)=24 AND dirty=false. Catches the "I forgot
		...
		t.Run(b.name, func(t *testing.T) {
			...
			assert.Equalf(t, wantHead, head,
				"schema_migrations.version on %s = %d; want %d "+
					"(matches the 24 .up.sql files committed in 000001..000024)",
