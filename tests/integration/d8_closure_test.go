...
//
//  1. All 19 migrations apply cleanly from an empty DB (no compound
//     failures, no partial-state crashes).
//  2. Every name in d1AcceptanceTablesPostgres (52 entries) is present
...
//  4. schema_migrations reports exactly 19 applied versions —
//     matches the 19 .up.sql files under
...
// Up() contains exactly the 52 names in d1AcceptanceTablesPostgres.
...
// expected version (20) with dirty=false. golang-migrate stores a
// single "current head" row (not a per-migration history), so the
// invariant is MAX(version)=20 AND dirty=false. Catches the "I forgot
		...
		t.Run(b.name, func(t *testing.T) {
			...
			assert.Equalf(t, wantHead, head,
				"schema_migrations.version on %s = %d; want %d "+
					"(matches the 20 .up.sql files committed in 000001..000020)",
