...
TMDB на этапе enrichment и кешируются в подключаемом object-хранилище.
Для self-hosting поддерживаются два бэкенда:

- **`mediaStore.mode=s3`** — S3-совместимое хранилище (работает и
  SeaweedFS, и MinIO). Обязательно, если бэкенд крутится больше чем в
  одной реплике: все реплики делят один бакет.
- **`mediaStore.mode=fs`** — обычная директория на файловой системе,
  подложенная persistent volume'ом (PVC). Полноценный вариант для
  self-hoster'ов, которые не хотят поднимать S3/SeaweedFS: байты
  пишутся атомарно на PVC и переживают рестарты.

> **Ограничение single-replica (режим fs):** файловое хранилище живёт
> на одном ReadWriteOnce (RWO) PVC, который может смонтировать только
> один под за раз. **Поэтому режим `fs` требует `replicas=1`.** Больше
> одной реплики — только с `mode=s3`, где все реплики делят один бакет.

Оба бэкенда content-addressed и используют одинаковый layout ключей,
так что переезд между ними — это просто регидрация с TMDB (хранилище
это кэш — см. [Self-healing](#self-healing) ниже).
...
- Раз в неделю запускается garbage-collection sweep, который
  освобождает осиротевшие блобы — ассеты, на которые не ссылается ни
  один сериал и у которых `media_assets.last_access_at` старше cooldown
  (30д). Sweep store-agnostic: он удаляет через тот же интерфейс
  `Store`, поэтому освобождает место и на бэкенде `s3`, и на `fs`
  (PVC). Удаление блоба, которого в хранилище уже нет, — no-op, а row
  в БД дропается только после успешного удаления из хранилища, так что
  прерванный sweep повторится на следующей неделе.
...
Pre-commit hooks локально ловят регрессии, которые иначе попадают в CI:
`gofmt`, `go vet`, `golangci-lint`, `tsc --noEmit`, дрейф OpenAPI, дрейф
i18n-ключей, ESLint и `go test -short`. Гейт **только локальный** — CI
pre-commit не запускает, он остаётся независимым ground truth.

Установка после клона (один раз):

```sh
brew install pre-commit       # или: pip install pre-commit
make pre-commit-install        # ставит .git/hooks/{pre-commit,pre-push}
```

Каждый `git commit` запускает быстрые hook'и (gofmt, vet, golangci-lint
по изменённым пакетам, eslint по staged TS, tsc при изменении web,
проверки дрейфа OpenAPI и i18n). Каждый `git push` дополнительно гонит
`go test -short ./...`, чтобы красные сборки не доезжали до remote.

Прогнать весь набор вручную:

```sh
make pre-commit-run
```

`git commit --no-verify` запрещён политикой репозитория (см. CLAUDE.md
"NEVER skip hooks").
