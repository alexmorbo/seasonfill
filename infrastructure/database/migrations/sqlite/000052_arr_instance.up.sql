...
--
-- SQLite stores FK targets by name in each child's CREATE text and, with
-- foreign_keys=off, ALTER TABLE RENAME does NOT rewrite them, so the
-- parent and every child that references it must be rebuilt inside a
-- foreign_keys=off envelope (mirrors 000015/000017/000020/000037).
-- Parent is rebuilt first (still referencing instance_secret by its
-- unchanged name), then instance_secret, then the other 12 children —
-- handling the cyclic arr_instance <-> instance_secret pair exactly like
-- 000020. atlas migrate diff cannot see the rename (emits a data-losing
-- drop+create) so this file is hand-authored; the resulting schema is
-- proven by internal/shared/db/migration_000052_roundtrip_test.go.
-- disable the enforcement of foreign-keys constraints
PRAGMA foreign_keys = off;

-- rebuild parent: sonarr_instance -> arr_instance, +type column, preserve rows.
CREATE TABLE `new_arr_instance` (
  `name` text NOT NULL,
  `url` text NOT NULL,
  ...
  PRIMARY KEY (`name`),
  CONSTRAINT `arr_instance_token_secret_id_fkey` FOREIGN KEY (`token_secret_id`) REFERENCES `instance_secret` (`id`) ON UPDATE NO ACTION ON DELETE SET NULL
);
...
-- rebuild instance_secret (cyclic child) -> FK arr_instance
CREATE TABLE `new_instance_secret` (
  ...
  `created_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
  `updated_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
  CONSTRAINT `instance_secret_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
CREATE TABLE `new_user_instance_tags` (
  ...
  CONSTRAINT `user_instance_tags_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE,
...
CREATE TABLE `new_grab_records` (
  ...
  `updated_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
  PRIMARY KEY (`id`),
  CONSTRAINT `grab_records_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE,
...
-- rebuild download_links -> FK arr_instance (series FK unchanged)
CREATE TABLE `new_download_links` (
  ...
  CONSTRAINT `download_links_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE,
...
-- rebuild watchdog_state -> FK arr_instance
CREATE TABLE `new_watchdog_state` (
  ...
  CONSTRAINT `watchdog_state_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild watchdog_blacklist -> FK arr_instance
CREATE TABLE `new_watchdog_blacklist` (
  ...
  PRIMARY KEY (`instance_name`, `sonarr_series_id`, `season_number`),
  CONSTRAINT `watchdog_blacklist_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild sonarr_instance_settings -> FK arr_instance (table name STAYS)
CREATE TABLE `new_sonarr_instance_settings` (
  ...
  CONSTRAINT `sonarr_instance_settings_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild decisions -> FK arr_instance
CREATE TABLE `new_decisions` (
  ...
  `created_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
  PRIMARY KEY (`id`),
  CONSTRAINT `decisions_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild origin_releases -> FK arr_instance
CREATE TABLE `new_origin_releases` (
  ...
  CONSTRAINT `origin_releases_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild qbit_settings -> FK arr_instance
CREATE TABLE `new_qbit_settings` (
  ...
  `updated_at` datetime NOT NULL DEFAULT (CURRENT_TIMESTAMP),
  PRIMARY KEY (`instance_name`),
  CONSTRAINT `qbit_settings_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild qbit_torrents -> FK arr_instance
CREATE TABLE `new_qbit_torrents` (
  ...
  CONSTRAINT `qbit_torrents_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild qbit_torrent_events -> FK arr_instance
CREATE TABLE `new_qbit_torrent_events` (
  ...
  CONSTRAINT `qbit_torrent_events_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- rebuild torrent_series_map -> FK arr_instance
CREATE TABLE `new_torrent_series_map` (
  ...
  CONSTRAINT `torrent_series_map_instance_name_fkey` FOREIGN KEY (`instance_name`) REFERENCES `arr_instance` (`name`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
-- create empty radarr settings sibling (mirrors sonarr_instance_settings shape)
