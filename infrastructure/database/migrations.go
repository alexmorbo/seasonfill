...
func stampBaselineIfNeeded(ctx context.Context, sqlDB *sql.DB, dialect string) error {
	...
	// 203 (B-1a): Detect v26 by checking for the series table.
	hasV26, err := tableExists(ctx, sqlDB, dialect, "series")
	if err != nil {
		return err
	}
	if hasV26 {
	...
	// 204 (B-2a): Detect v27 by checking for the people table.
	hasV27, err := tableExists(ctx, sqlDB, dialect, "people")
	if err != nil {
		return err
	}
	if hasV27 {
	...
	// 205 (B-2b): Detect v28 by checking for the genres table.
	hasV28, err := tableExists(ctx, sqlDB, dialect, "genres")
	if err != nil {
		return err
	}
	if hasV28 {
	...
	// 206 (B-3): Detect v29 by checking for the videos table.
	hasV29, err := tableExists(ctx, sqlDB, dialect, "videos")
	if err != nil {
		return err
	}
	if hasV29 {
	...
	// 206 (B-3): Detect v30 by checking for the person_credits table.
	hasV30, err := tableExists(ctx, sqlDB, dialect, "person_credits")
	if err != nil {
		return err
	}
	if hasV30 {
	...
	// 206 (B-3): Detect v31 by checking for the sync_log table.
	hasV31, err := tableExists(ctx, sqlDB, dialect, "sync_log")
	if err != nil {
		return err
	}
	if hasV31 {
	...
	// 219 (A-1): Detect v35 by checking for the qbit_torrents table.
	hasV35, err := tableExists(ctx, sqlDB, dialect, "qbit_torrents")
	if err != nil {
		return err
	}
	if hasV35 {
	...
	// 301: Detect v36 by checking for the app_settings table.
	hasV36, err := tableExists(ctx, sqlDB, dialect, "app_settings")
	if err != nil {
		return err
	}
	if hasV36 {
	...
	// 305: Detect v37 by checking for the external_service_quota_state table.
	hasV37, err := tableExists(ctx, sqlDB, dialect, "external_service_quota_state")
	if err != nil {
		return err
	}
	if hasV37 {
	...
	// 307: Detect v38 by checking for person_credits.department.
	hasV38, err := columnExists(ctx, sqlDB, dialect, "person_credits", "department")
	if err != nil {
		return err
	}
	if hasV38 {
	...
	// 308: Detect v39 by checking for qbit_torrents.season_number.
	hasV39, err := columnExists(ctx, sqlDB, dialect, "qbit_torrents", "season_number")
	if err != nil {
		return err
	}
	if hasV39 {
	...
	// Story 374: v42 = series_cache.episode_file_count exists.
	hasV42, err := columnExists(ctx, sqlDB, dialect, "series_cache", "episode_file_count")
	if err != nil {
		return err
	}
	if hasV42 {
	...
	// Story 376: v43 = series_cache.aired_episode_count exists.
	hasV43, err := columnExists(ctx, sqlDB, dialect, "series_cache", "aired_episode_count")
	if err != nil {
		return err
	}
	if hasV43 {
