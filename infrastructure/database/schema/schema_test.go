...
// count is 55 after 584a series_media_texts: 52 prior (D-7 media_assets) +
// 1 (N-2a discovery_lists) + 1 (B3a season_texts) + 1 (584a
// series_media_texts, always-on i18n text table alongside
...
func TestSchemaCoverage_BothDialects(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		t.Run(string(d), func(t *testing.T) {
			...
			if len(s.Tables) != 55 {
				t.Fatalf("Schema(%q) tables = %d, want 55 (after 584a series_media_texts)", d, len(s.Tables))
			}
		})
	}
}
...
func TestSchemaCoverage_PeopleSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 52 {
		t.Fatalf("Schema(postgres) with skip people tables = %d, want 52 (55 - 3 people)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_LoadHonorsEnv(t *testing.T) {
	...
	if len(s.Tables) != 55 {
		t.Fatalf("Load() tables = %d, want 55 (after 584a series_media_texts)", len(s.Tables))
	}
}
