...
// count is 28 after D-1-5: 24 prior + 3 instance projections
// (series_cache, episode_states, season_stats) + 1 enrichment_errors.
func TestSchemaCoverage_BothDialects(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		t.Run(string(d), func(t *testing.T) {
			...
			if len(s.Tables) != 28 {
				t.Fatalf("Schema(%q) tables = %d, want 28", d, len(s.Tables))
			}
		})
	}
}
...
// when unset, all 28 tables are present (the prod path). 28 - 4 = 24.
func TestSchemaCoverage_TaxonomySkipFlag(t *testing.T) {
	t.Setenv("ATLAS_SCHEMA_SKIP_TAXONOMY_JOINS", "1")
	s := Schema(DialectPostgres)
	if len(s.Tables) != 24 {
		t.Fatalf("Schema(postgres) with skip flag tables = %d, want 24 (28 - 4 joins)", len(s.Tables))
	}
...
// pre-existing migrations); when unset, all 28 tables are present (the
// prod path). 28 default - 3 (skipped people) = 25.
func TestSchemaCoverage_PeopleSkipFlag(t *testing.T) {
	t.Setenv("ATLAS_SCHEMA_SKIP_PEOPLE", "1")
	s := Schema(DialectPostgres)
	if len(s.Tables) != 25 {
		t.Fatalf("Schema(postgres) with skip people tables = %d, want 25 (28 - 3 people)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
// unset, all 28 tables are present (the prod path). 28 - 4 = 24.
func TestSchemaCoverage_SeriesExtrasSkipFlag(t *testing.T) {
	t.Setenv("ATLAS_SCHEMA_SKIP_SERIES_EXTRAS", "1")
	s := Schema(DialectPostgres)
	if len(s.Tables) != 24 {
		t.Fatalf("Schema(postgres) with skip series_extras tables = %d, want 24 (28 - 4 extras)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_LoadHonorsEnv(t *testing.T) {
	...
	if len(s.Tables) != 28 {
		t.Fatalf("Load() tables = %d, want 28", len(s.Tables))
	}
}
...
// 000007 cleanly); when unset, all 28 tables are present (the prod
// path). 28 - 3 = 25.
func TestSchemaCoverage_InstanceProjectionsSkipFlag(t *testing.T) {
	t.Setenv("ATLAS_SCHEMA_SKIP_INSTANCE_PROJECTIONS", "1")
	s := Schema(DialectPostgres)
	if len(s.Tables) != 25 {
		t.Fatalf("Schema(postgres) with skip projections tables = %d, want 25 (28 - 3 projections)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
// to generate 000008 cleanly); when unset, all 28 tables are present
// (the prod path). 28 - 1 = 27.
func TestSchemaCoverage_EnrichmentTrackingSkipFlag(t *testing.T) {
	t.Setenv("ATLAS_SCHEMA_SKIP_ENRICHMENT_TRACKING", "1")
	s := Schema(DialectPostgres)
	if len(s.Tables) != 27 {
		t.Fatalf("Schema(postgres) with skip enrichment tables = %d, want 27 (28 - 1 enrichment_errors)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
