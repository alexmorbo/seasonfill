...
// count is 54 after E-1 B3a season_texts: 52 prior (D-7 media_assets) +
// 1 (N-2a discovery_lists) + 1 (B3a season_texts, always-on i18n text
// table alongside series_texts/episode_texts).
func TestSchemaCoverage_BothDialects(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		t.Run(string(d), func(t *testing.T) {
			...
			if len(s.Tables) != 54 {
				t.Fatalf("Schema(%q) tables = %d, want 54 (after E-1 B3a season_texts)", d, len(s.Tables))
			}
		})
	}
}
...
func TestSchemaCoverage_LoadHonorsEnv(t *testing.T) {
	...
	if len(s.Tables) != 54 {
		t.Fatalf("Load() tables = %d, want 54 (after E-1 B3a season_texts)", len(s.Tables))
	}
}
