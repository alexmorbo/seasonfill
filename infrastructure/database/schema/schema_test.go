...
func TestSchemaCoverage_BothDialects(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		t.Run(string(d), func(t *testing.T) {
			...
			if len(s.Tables) != 66 {
				t.Fatalf("Schema(%q) tables = %d, want 66 (after Ф6-R-1 radarr_instance_settings)", d, len(s.Tables))
			}
		})
	}
}
...
func TestSchemaCoverage_TaxonomySkipFlag(t *testing.T) {
	...
	if len(s.Tables) != 62 {
		t.Fatalf("Schema(postgres) with skip flag tables = %d, want 62 (66 - 4 joins)", len(s.Tables))
	}
...
func TestSchemaCoverage_PeopleSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 61 {
		t.Fatalf("Schema(postgres) with skip people tables = %d, want 61 (66 - 5 people)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_SeriesExtrasSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 61 {
		t.Fatalf("Schema(postgres) with skip series_extras tables = %d, want 61 (66 - 5 extras)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_LoadHonorsEnv(t *testing.T) {
	...
	if len(s.Tables) != 66 {
		t.Fatalf("Load() tables = %d, want 66 (after Ф6-R-1 radarr_instance_settings)", len(s.Tables))
	}
}
...
func TestSchemaCoverage_InstanceProjectionsSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 63 {
		t.Fatalf("Schema(postgres) with skip projections tables = %d, want 63 (66 - 3 projections)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_EnrichmentTrackingSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 65 {
		t.Fatalf("Schema(postgres) with skip enrichment tables = %d, want 65 (66 - 1 enrichment_errors)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
...
func TestSchemaCoverage_SeriesImagesSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 65 {
		t.Fatalf("Schema(postgres) with skip series_images tables = %d, want 65 (66 - 1 series_images)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
...
func TestSchemaCoverage_AdminSkipFlag(t *testing.T) {
	...
	if len(s.Tables) != 43 {
		t.Fatalf("Schema(postgres) with skip admin+auth+app_config+grab+watchdog+grab_audit tables = %d, want 43 (65 - 5 admin - 2 auth - 3 app_config - 3 grab - 2 watchdog - 3 grab_audit - 4 qbit_runtime)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_AuthSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 64 {
		t.Fatalf("Schema(postgres) with skip auth tables = %d, want 64 (66 - 2 auth)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_GrabSkipFlag(t *testing.T) {
	...
	s := Schema(DialectPostgres)
	if len(s.Tables) != 63 {
		t.Fatalf("Schema(postgres) with skip grab tables = %d, want 63 (66 - 3 grab)", len(s.Tables))
	}
	for _, tbl := range s.Tables {
		switch tbl.Name {
...
func TestSchemaCoverage_WatchdogSkipFlag(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		s := Schema(d)
		if len(s.Tables) != 64 {
			t.Fatalf("with skip set: Schema(%q) tables = %d, want 64 (66 - 2 watchdog)", d, len(s.Tables))
		}
		for _, tbl := range s.Tables {
...
func TestSchemaCoverage_GrabAuditSkipFlag(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		s := Schema(d)
		if len(s.Tables) != 63 {
			t.Fatalf("with skip set: Schema(%q) tables = %d, want 63 (66 - 3 grab_audit)", d, len(s.Tables))
		}
		for _, tbl := range s.Tables {
			switch tbl.Name {
...
func TestSchemaCoverage_ScanRunsSkipFlag(t *testing.T) {
	...
	for _, d := range []Dialect{DialectPostgres, DialectSQLite} {
		s := Schema(d)
		if len(s.Tables) != 65 {
			t.Fatalf("with skip set: Schema(%q) tables = %d, want 65 (66 - 1 scan_runs)", d, len(s.Tables))
		}
		for _, tbl := range s.Tables {
