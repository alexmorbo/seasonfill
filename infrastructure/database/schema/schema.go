...
func buildAppConfigTable(d Dialect) *atlasschema.Table {
	...
	apiKeyAutoGenerated := atlasschema.NewBoolColumn("api_key_auto_generated", "boolean").
		SetNull(false).SetDefault(&atlasschema.Literal{V: "false"})
	// M1 (ADR-0014 §П25) — global media direct/proxy toggle. false =
	// proxy (stream blobs through this pod, today's default); true =
	// direct (302 the browser to the TMDB source_url). Hot-reloaded via
	// the runtime-config bus; read per-request by the media handler.
	mediaDirect := atlasschema.NewBoolColumn("media_direct", "boolean").
		SetNull(false).SetDefault(&atlasschema.Literal{V: "false"})
	// ADR-0015 Ф3 S3 (F-14) — revocation generation for the signed ICS
	// calendar-feed token. Separate from auth_session_epoch so revoking a
	// leaked feed URL does not log out browser sessions.
	icsEpoch := atlasschema.NewIntColumn("ics_epoch", "bigint").
		SetNull(false).SetDefault(&atlasschema.Literal{V: "0"})
